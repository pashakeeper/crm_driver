    ...
    try {
      const response = await axios.get(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
          address
        )}.json?access_token=pk.eyJ1IjoiYmF0MzMzIiwiYSI6ImNtMGxoczk1cTA0emUycnF1anB0aGF4dW4ifQ.cirY7NRQxoPAfDuyAOUn-g`
      );
  ...
  const calculateDistance = async (origin, destination) => {
    try {
      ...
      if (originCoordinates && destinationCoordinates) {
        const response = await axios.get(
          `https://api.mapbox.com/directions/v5/mapbox/driving/${originCoordinates[0]},${originCoordinates[1]};${destinationCoordinates[0]},${destinationCoordinates[1]}?access_token=pk.eyJ1IjoiYmF0MzMzIiwiYSI6ImNtMGxoczk1cTA0emUycnF1anB0aGF4dW4ifQ.cirY7NRQxoPAfDuyAOUn-g`
        );
